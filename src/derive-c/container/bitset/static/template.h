...
PUBLIC static SELF NS(SELF, new)() {
    return (SELF){
        .bits = {},
...
PUBLIC static bool NS(SELF, try_set)(SELF* self, INDEX_TYPE index, bool value) {
...
PUBLIC static void NS(SELF, set)(SELF* self, INDEX_TYPE index, bool value) {
    INVARIANT_CHECK(self);
    DC_ASSERT(NS(SELF, try_set)(self, index, value));
}

PUBLIC static bool NS(SELF, get)(SELF const* self, INDEX_TYPE index) {
    INVARIANT_CHECK(self);
...
PUBLIC static void NS(SELF, delete)(SELF* self) { INVARIANT_CHECK(self); }
...
PUBLIC static bool NS(ITER_CONST, empty_item)(ITER_INDEX_TYPE const* item) {
    return *item == EXCLUSIVE_END_INDEX;
}

PUBLIC static ITER_INDEX_TYPE NS(ITER_CONST, next)(ITER_CONST* iter) {
    DC_ASSUME(iter);
...
PUBLIC static bool NS(ITER, empty_item)(ITER_INDEX_TYPE const* item) {
    return *item == EXCLUSIVE_END_INDEX;
}

PUBLIC static ITER_INDEX_TYPE NS(ITER, next)(ITER* iter) {
    DC_ASSUME(iter);
    mutation_version_check(&iter->version);
