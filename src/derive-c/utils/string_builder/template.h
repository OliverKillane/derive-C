...
#include <derive-c/core/includes/def.h>
    ...
    #error "the gnu source macro must be defined to use cookie io"
#endif
...
static ssize_t PRIV(NS(SELF, write))(void* capture, const char* data, size_t size) {
    ...
    if (self->size_without_null + size + 1 > self->capacity) {
        ...
        if (self->buf != NULL) {
            ...
            new_buf = (char*)NS(ALLOC, reallocate)(self->alloc_ref, self->buf, self->capacity,
                                                   new_capacity);
        } else {
...
DC_PUBLIC static FILE* NS(SELF, stream)(SELF* self) {
    INVARIANT_CHECK(self);
...
DC_PUBLIC static char const* NS(SELF, string)(SELF const* self) {
    INVARIANT_CHECK(self);
...
/// string builder.
DC_PUBLIC static char* NS(SELF, release_string)(SELF* self) {
    INVARIANT_CHECK(self);
    char* buf = self->buf;
    self->size_without_null = 0;
    self->buf = NULL;
...
DC_PUBLIC static size_t NS(SELF, string_size)(SELF* self) {
    INVARIANT_CHECK(self);
