...
#define ID(type) \
    static size_t hash_id_##type(type const* key) { \
        _Static_assert(sizeof(type) <= sizeof(size_t), "ID hashing only supports up to size_t integers"); \
        return (size_t)(*key); \
    }

/// Using [MurmurHash3's finalizer](https://github.com/aappleby/smhasher/blob/0ff96f7835817a27d0487325b6c16033e2992eb5/src/MurmurHash3.cpp#L81)
/// for integer hashing.
...
#define MURMURHASH_3_FMIx64(type) \
    static size_t hash_murmurhash3_##type(type const* key) { \
        _Static_assert(sizeof(type) <= sizeof(uint64_t), "MurmurHash3 only supports up to 64-bit integers"); \
