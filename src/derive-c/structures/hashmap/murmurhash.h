...
void derive_c_MurmurHash3_x86_32 ( const void * key, int len, uint32_t seed, void * out )
{
  ...
  case 2: k1 ^= tail[1] << 8;
  case 1: k1 ^= tail[0];
          k1 *= c1; k1 = derive_c_rotl32(k1,15); k1 *= c2; h1 ^= k1;
  };
...
void derive_c_MurmurHash3_x86_128 (const void * key, const int len, uint32_t seed, void * out )
{
  ...
  for(int i = -nblocks; i; i++)
  {
    ...
    h1 = derive_c_rotl32(h1,19); h1 += h2; h1 = h1*5+0x561ccd1b;

    k2 *= c2; k2  = derive_c_rotl32(k2,16); k2 *= c3; h2 ^= k2;

    h2 = derive_c_rotl32(h2,17); h2 += h3; h2 = h2*5+0x0bcaa747;

    k3 *= c3; k3  = derive_c_rotl32(k3,17); k3 *= c4; h3 ^= k3;

    h3 = derive_c_rotl32(h3,15); h3 += h4; h3 = h3*5+0x96cd1c35;

    k4 *= c4; k4  = derive_c_rotl32(k4,18); k4 *= c1; h4 ^= k4;

    h4 = derive_c_rotl32(h4,13); h4 += h1; h4 = h4*5+0x32ac3b17;
  }
...
void derive_c_MurmurHash3_x64_128 ( const void * key, const int len,
                           const uint32_t seed, void * out )
{
  ...
  for(int i = 0; i < nblocks; i++)
  {
    ...
    k1 *= c1; k1  = derive_c_rotl64(k1,31); k1 *= c2; h1 ^= k1;

    h1 = derive_c_rotl64(h1,27); h1 += h2; h1 = h1*5+0x52dce729;

    k2 *= c2; k2  = derive_c_rotl64(k2,33); k2 *= c1; h2 ^= k2;

    h2 = derive_c_rotl64(h2,31); h2 += h1; h2 = h2*5+0x38495ab5;
  }
