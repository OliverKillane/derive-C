...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate .vscode/settings.json for clangd"
    )
    parser.add_argument(
        "--vscode-dir", type=Path, required=True, help="Path to .vscode directory"
    )
    parser.add_argument(
        "--clangd-path", required=True, help="Path to clangd executable"
    )
    parser.add_argument(
        "--clang-path",
        required=True,
        help="Path to clang executable (for query-driver)",
    )
    parser.add_argument(
        "--resource-dir", required=True, help="Path to clang resource directory"
    )
    parser.add_argument(
